...
void IntervalTree::balanceTree(Node* x,Node* y, Node* z,bool isRight1,bool isRight2){
  if(isRight1){
    if(isRight2){
      ...
      updateNode(z,false);
      if(z == this->root) this->root = y;
    } else {
      right_left_Solve(x,y,z);
      updateNode(y,false);
      updateNode(z,false);
      if(z == this->root) this->root = x;
    }
  } else {
    if(isRight2){
...
void IntervalTree::swapAndDestroy(Node *father,Node *child, bool areClose,bool isLeft){
  if(this->root == father)this->root = child;
  if(areClose){
...
bool IntervalTree::intersection(Node* currentNode,int ymin, int ymax,int xmin,int xmax){
  //One of the two rectangles is above the top edge of the other rectangle
  if(currentNode->intervalY.second < ymin || currentNode->intervalY.first > ymax){
    return false;
  }
  //One of the two rectangles is on the left side of the left edge of the other rectangle
  if(currentNode->intervalX.second < xmin || currentNode->intervalX.first > xmax){
    return false;
  }
  return true;
}
bool IntervalTree::completeIntersection(Node* currentNode,int ymin, int ymax,int xmin,int xmax){
  if(currentNode->intervalX.first <= xmin && currentNode->intervalX.second >= xmax &&
     currentNode->intervalY.second >= ymax && currentNode->intervalY.first <= ymin){
       std::cout << "Greater rectangle: " << '\n';
       printNode(currentNode);
       std::cout << "little rectangle" << '\n';
       std::cout << ymin<<' '<<ymax<<' '<<' '<<xmin<<' '<<xmax << '\n';
       return true;
     }
  if(currentNode->intervalX.first >= xmin && currentNode->intervalX.second <= xmax &&
     currentNode->intervalY.second <= ymax && currentNode->intervalY.first >= ymin){
...
bool IntervalTree::search(int ymin, int ymax,int xmin,int xmax, Node* currentNode){
    if(currentNode != NULL){
        if(intersection(currentNode,ymin,ymax,xmin,xmax)){
          std::cout << "FOUND INTERSECTION: " << '\n';
          printNode(currentNode);
          std::cout<<'\n' << "another square (xmin-max,ymin-max): "<<ymin << ' '<<ymax<< ' '<<xmin << ' '<<xmax << '\n';
          if(completeIntersection(currentNode,ymin,ymax,xmin,xmax)){
